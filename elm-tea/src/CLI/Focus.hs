...
getFocusPosition =
  let containerFocus layout alignment children i cfocus =
        let position LayoutRow AlignCenter = Layout.rowPositions
            position LayoutColumn AlignCenter = Layout.columnPositions
            position LayoutColumn AlignStart = Layout.leftAlignedColumnPositions
            position _ _ = Layout.rowPositions -- TODO
         in position layout alignment children & List.drop i & List.head &
            Maybe.andThen
              (\((cx, cy), child) ->
